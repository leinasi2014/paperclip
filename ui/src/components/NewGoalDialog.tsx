...
export function NewGoalDialog() {
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error(t("errors.noCompanySelected"));
  ...
  const levelLabel = (value: string) => t(`dialogs.newGoal.levels.${value}`);
  const statusLabel = (value: string) => t(`dialogs.newGoal.statuses.${value}`);
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{newGoalDefaults.parentId ? t("dialogs.newGoal.headerSubGoal") : t("dialogs.newGoal.headerGoal")}</span>
          </div>
        ...
        <div className="flex items-center justify-end px-4 py-2.5 border-t border-border">
          ...
          >
            {createGoal.isPending
              ? t("dialogs.newGoal.creating")
              : newGoalDefaults.parentId
                ? t("dialogs.newGoal.createSubGoal")
                : t("dialogs.newGoal.createGoal")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
