...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) {
        throw new Error(
          t("config.form.errors.selectCompanyToCreateSecrets", {
            ns: "agents",
  ...
  const uploadMarkdownImage = useMutation({
    mutationFn: async ({ file, namespace }: { file: File; namespace: string }) => {
      if (!selectedCompanyId) {
        throw new Error(
          t("pages.agentDetail.prompts.errors.selectCompanyToUploadImages", {
            ns: "agents",
            defaultValue: "Select a company to upload images",
          }),
        );
      }
  ...
  const {
    ...
    queryFn: () => {
      if (!selectedCompanyId) {
        throw new Error(
          t("config.form.errors.selectCompanyToDetectHermesModel", {
            ns: "agents",
            defaultValue: "Select a company to detect the Hermes model",
          }),
        );
      }
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error(
          t("config.form.errors.selectCompanyToTestEnvironment", {
            ns: "agents",
            defaultValue: "Select a company to test adapter environment",
          }),
        );
      }
  ...
  const localizedThinkingEffortOptions = useMemo(
    () =>
      thinkingEffortOptions.map((option) => ({
        ...option,
        label: t(`config.form.optionLabels.${option.id || "auto"}`, {
          ns: "agents",
    ...
    <div className={cn("relative", cards && "space-y-6")}>
        ...
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">
              {t("config.form.sections.unsavedChanges", {
                ns: "agents",
            ...
            >
              {!isCreate && props.isSaving
                ? t("pages.agentDetail.actions.saving", {
                    ns: "agents",
                    defaultValue: "Saving...",
                  })
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("config.form.sections.identity", { ns: "agents", defaultValue: "Identity" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("config.form.sections.identity", { ns: "agents", defaultValue: "Identity" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("config.form.fields.name", { ns: "agents", defaultValue: "Name" })} hint={help.name}>
              <DraftInput
                value={eff("identity", "name", props.agent.name)}
                onCommit={(v) => mark("identity", "name", v)}
                immediate
                className={inputClass}
                placeholder={t("config.form.placeholders.agentName", {
                  ns: "agents",
                  defaultValue: "Agent name",
                })}
              />
            </Field>
            <Field label={t("config.form.fields.title", { ns: "agents", defaultValue: "Title" })} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("config.form.placeholders.agentTitle", {
                  ns: "agents",
                  defaultValue: "e.g. VP of Engineering",
                })}
              />
            </Field>
            <Field label={t("config.form.fields.reportsTo", { ns: "agents", defaultValue: "Reports to" })} hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel={t("config.form.actions.chooseManager", {
                  ns: "agents",
                  defaultValue: "Choose manager…",
                })}
              />
            </Field>
            <Field label={t("config.form.fields.capabilities", { ns: "agents", defaultValue: "Capabilities" })} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "")}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("config.form.placeholders.capabilities", {
                  ns: "agents",
                  defaultValue: "Describe what this agent can do...",
                })}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("config.form.fields.promptTemplate", { ns: "agents", defaultValue: "Prompt Template" })} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("config.form.placeholders.promptTemplate", {
                      ns: "agents",
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("config.form.notices.promptTemplateReplay", {
                    ns: "agents",
                    defaultValue: "Prompt template is replayed on every heartbeat. Keep it compact and dynamic to avoid recurring token cost and cache churn.",
                  })}
                </div>
              </>
            )}
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("config.form.sections.adapter", { ns: "agents", defaultValue: "Adapter" })}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("config.form.sections.adapter", { ns: "agents", defaultValue: "Adapter" })}</span>
          }
          {showAdapterTestEnvironmentButton && (
            ...
            >
              {testEnvironment.isPending
                ? t("config.form.actions.testing", { ns: "agents", defaultValue: "Testing..." })
                : t("config.form.actions.testEnvironment", { ns: "agents", defaultValue: "Test environment" })}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("config.form.fields.adapterType", { ns: "agents", defaultValue: "Adapter type" })} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
                onChange={(t) => {
                  if (isCreate) {
          ...
          {testEnvironment.error && (
            <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              {testEnvironment.error instanceof Error
                ...
                : t("config.form.errors.environmentTestFailed", {
                    ns: "agents",
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("config.form.fields.workingDirectoryDeprecated", { ns: "agents", defaultValue: "Working directory (deprecated)" })} hint={help.cwd}>
              <div className="flex items-center gap-2 rounded-md border border-border px-2.5 py-1.5">
                ...
                <DraftInput
                  ...
                  placeholder={t("config.form.placeholders.workingDirectory", {
                    ns: "agents",
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                {t("config.form.notices.promptTemplateCreateReplay", {
                  ns: "agents",
                  defaultValue: "Prompt template is replayed on every heartbeat. Prefer small task framing and variables like {{context}} or {{run}}; avoid repeating stable instructions here.",
                  context: "{{ context.* }}",
                  run: "{{ run.* }}",
                })}
              </div>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("config.form.sections.permissionsAndConfiguration", { ns: "agents", defaultValue: "Permissions & Configuration" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("config.form.sections.permissionsAndConfiguration", { ns: "agents", defaultValue: "Permissions & Configuration" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("config.form.fields.command", { ns: "agents", defaultValue: "Command" })} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              <ModelDropdown
                ...
                detectModelLabel={adapterType === "hermes_local"
                  ? t("config.form.actions.detectFromHermesConfig", {
                      ns: "agents",
                      defaultValue: "Detect from Hermes config",
                    })
                  : undefined}
              />
              {fetchedModelsError && (
                <p className="text-xs text-destructive">
                  {fetchedModelsError instanceof Error
                    ...
                    : t("config.form.errors.failedToLoadAdapterModels", {
                        ns: "agents",
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("config.form.notices.codexSearchMinimal", {
                          ns: "agents",
                          defaultValue: "Codex may reject `minimal` thinking when search is enabled.",
                        })}
                      </p>
                    )}
                </>
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("config.form.fields.bootstrapPromptLegacy", { ns: "agents", defaultValue: "Bootstrap prompt (legacy)" })} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                        "bootstrapPromptTemplate",
                      ...
                      placeholder={t("config.form.placeholders.bootstrapPrompt", {
                        ns: "agents",
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("config.form.notices.bootstrapPromptLegacy", {
                      ns: "agents",
                      defaultValue: "Bootstrap prompt is legacy and will be removed in a future release. Consider moving this content into the agent's prompt template or instructions file instead.",
                    })}
                  </div>
                </>
              )}
              ...
              <Field label={t("config.form.fields.extraArgs", { ns: "agents", defaultValue: "Extra args (comma-separated)" })} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("config.form.placeholders.extraArgs", {
                    ns: "agents",
              ...
              <Field label={t("config.form.fields.environmentVariables", { ns: "agents", defaultValue: "Environment variables" })} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("config.form.fields.timeoutSec", { ns: "agents", defaultValue: "Timeout (sec)" })} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("config.form.fields.graceSec", { ns: "agents", defaultValue: "Interrupt grace period (sec)" })} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("config.form.sections.runPolicy", { ns: "agents", defaultValue: "Run Policy" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("config.form.sections.runPolicy", { ns: "agents", defaultValue: "Run Policy" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("config.form.fields.heartbeatOnInterval", { ns: "agents", defaultValue: "Heartbeat on interval" })}
              hint={help.heartbeatInterval}
              checked={val!.heartbeatEnabled}
              onCheckedChange={(v) => set!({ heartbeatEnabled: v })}
              number={val!.intervalSec}
              onNumberChange={(v) => set!({ intervalSec: v })}
              numberLabel={t("config.form.labels.secondsShort", { ns: "agents", defaultValue: "sec" })}
              numberPrefix={t("config.form.labels.runHeartbeatEvery", { ns: "agents", defaultValue: "Run heartbeat every" })}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("config.form.sections.advancedRunPolicy", { ns: "agents", defaultValue: "Advanced Run Policy" })}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("config.form.fields.wakeOnDemand", { ns: "agents", defaultValue: "Wake on demand" })}
              ...
              />
              <Field label={t("config.form.fields.cooldownSec", { ns: "agents", defaultValue: "Cooldown (sec)" })} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("config.form.fields.maxConcurrentRuns", { ns: "agents", defaultValue: "Max concurrent runs" })} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  ...
  const statusLabel =
    result.status === "pass"
      ? t("config.form.status.passed", { defaultValue: "Passed" })
      : result.status === "warn"
        ? t("config.form.status.warnings", { defaultValue: "Warnings" })
        : t("config.form.status.failed", { defaultValue: "Failed" });
  const statusClass =
    result.status === "pass"
          ...
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && (
              <span className="block opacity-90 break-words">
                {t("config.form.labels.hintPrefix", { defaultValue: "Hint:" })} {check.hint}
              </span>
            )}
          </div>
        ))}
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">
                {t("config.form.labels.comingSoon", { defaultValue: "Coming soon" })}
              </span>
            )}
          </button>
        ))}
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    const name = window.prompt(
      t("config.form.fields.secretName", {
    ...
    try {
      ...
      setSealError(
        err instanceof Error
          ? err.message
          : t("config.form.errors.failedToCreateSecret", {
  ...
  return (
    <div className="space-y-1.5">
        ...
        return (
          <div key={i} className="flex items-center gap-1.5">
            <input
              ...
              placeholder={t("config.form.placeholders.envKey", { defaultValue: "KEY" })}
              value={row.key}
            ...
            >
              <option value="plain">{t("config.form.labels.plain", { defaultValue: "Plain" })}</option>
              <option value="secret">{t("config.form.labels.secret", { defaultValue: "Secret" })}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                >
                  <option value="">
                    {t("config.form.placeholders.selectSecret", { defaultValue: "Select secret..." })}
                  </option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title={t("config.form.actions.createSecretFromPlainValue", {
                    defaultValue: "Create secret from current plain value",
                  })}
                >
                  {t("config.form.actions.new", { defaultValue: "New" })}
                </button>
              </>
            ) : (
              <>
                <input
                  ...
                  placeholder={t("config.form.placeholders.envValue", { defaultValue: "value" })}
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("config.form.actions.storeValueAsSecret", {
                    defaultValue: "Store value as secret and replace with reference",
                  })}
                >
                  {t("config.form.actions.seal", { defaultValue: "Seal" })}
                </button>
              </>
            )}
            {!isTrailing ? (
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("config.form.notices.envVarNote", {
...
function ModelDropdown({
  ...
  return (
    <Field label={t("config.form.fields.model", { defaultValue: "Model" })} hint={help.model}>
      <Popover
        open={open}
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ...
                : value || (
                  allowDefault
                    ? t("config.form.placeholders.defaultModel", { defaultValue: "Default" })
                    : required
                      ? t("config.form.placeholders.selectModelRequired", { defaultValue: "Select model (required)" })
                      : t("config.form.placeholders.selectModel", { defaultValue: "Select model" })
                )}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable
                ? t("config.form.placeholders.searchModelsCreatable", {
                    defaultValue: "Search models... (type to create)",
                  })
                : t("config.form.placeholders.searchModels", {
          ...
          {onDetectModel && !detectedModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel
                ? t("config.form.actions.detecting", { defaultValue: "Detecting..." })
                : (detectModelLabel ?? t("config.form.actions.detectFromConfig", { defaultValue: "Detect from config" }))}
            </button>
          )}
          {value && !models.some((m) => m.id === value) && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/20">
                {t("config.form.labels.current", { defaultValue: "current" })}
              </span>
            </button>
          )}
          {detectedModel && detectedModel !== value && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/20">
                {t("config.form.labels.detected", { defaultValue: "detected" })}
              </span>
            </button>
          )}
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {canCreateManualModel && (
              ...
              >
                <span>{t("config.form.actions.useManualModel", { defaultValue: "Use manual model" })}</span>
            ...
            {filteredModels.length === 0 && !canCreateManualModel && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? t("config.form.errors.noHermesModelDetected", {
                        defaultValue: "No Hermes model detected yet. Configure Hermes or enter a provider/model manually.",
                      })
                    : t("config.form.errors.noModelsFound", {
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("config.form.fields.thinkingEffort", { defaultValue: "Thinking effort" })} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected?.label ?? t("config.form.optionLabels.auto", { defaultValue: "Auto" })}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
