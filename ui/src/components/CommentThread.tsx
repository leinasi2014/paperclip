        ...
        <span className="flex items-center gap-1.5">
          ...
          {isPending ? (
            <span className="text-xs text-muted-foreground">
              {isQueued ? t("commentThread.queueing") : t("commentThread.sending")}
            </span>
          ) : (
            <a
...
const TimelineList = memo(function TimelineList({
  ...
  if (timeline.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("commentThread.emptyTimeline")}</p>;
  }
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold">
        {t("commentThread.title", { count: timeline.length + queuedComments.length })}
      </h3>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h4 className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-700 dark:text-amber-300">
              {t("commentThread.queuedTitle", { count: queuedComments.length })}
            </h4>
            {onInterruptQueued && queuedComments[0]?.queueTargetRunId ? (
              ...
              >
                {interruptingQueuedRunId === queuedComments[0].queueTargetRunId
                  ? t("commentThread.interrupting")
                  : t("commentThread.interrupt")}
              </Button>
            ) : null}
          </div>
          <div className="space-y-3">
                ...
                if (!option) {
                  return (
                    <span className="text-muted-foreground">
                      {t("commentThread.assigneePlaceholder")}
                    </span>
                  );
                }
