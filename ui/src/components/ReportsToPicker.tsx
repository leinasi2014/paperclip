...
export function ReportsToPicker({
  ...
  const effectiveDisabledEmptyLabel = disabledEmptyLabel ?? t("reportsToPicker.disabledEmptyLabel");
  const effectiveChooseLabel = chooseLabel ?? t("reportsToPicker.chooseLabel");
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            {t("reportsToPicker.missingSavedManager")}
          </div>
        )}
        {rows.map((a) => (
          <button
