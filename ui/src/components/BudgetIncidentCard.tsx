        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-red-200/80">
              {t("budgetIncident.scopeHardStop", {
      ...
      <CardContent className="space-y-4 px-5 pb-5 pt-0">
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? t("budgetIncident.paused.project", {
                  defaultValue: "Project execution is paused. New work in this project will not start until you resolve the budget incident.",
                })
              : t("budgetIncident.paused.scope", {
                  defaultValue: "This scope is paused. New heartbeats will not start until you resolve the budget incident.",
                })}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
            {t("budgetIncident.fields.newBudgetUsd", { defaultValue: "New budget (USD)" })}
          </label>
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating
                ? t("budgetIncident.actions.applying", { defaultValue: "Applying..." })
                : t("budgetIncident.actions.raiseAndResume", { defaultValue: "Raise budget & resume" })}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-200/80">
              {t("budgetIncident.errors.mustExceedObserved", {
        ...
        <div className="flex justify-end">
          <Button variant="ghost" className="text-muted-foreground" disabled={isMutating} onClick={onKeepPaused}>
            {t("budgetIncident.actions.keepPaused", { defaultValue: "Keep paused" })}
          </Button>
        </div>
      </CardContent>
    </Card>
