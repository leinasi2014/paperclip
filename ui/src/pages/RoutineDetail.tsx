...
function formatActivityDetailValue(value: unknown, t: (key: string) => string): string {
  if (value === null) return t("activity.value.null");
  if (typeof value === "string") return value;
  if (typeof value === "number" || typeof value === "boolean") return String(value);
  if (Array.isArray(value)) return value.length === 0 ? t("activity.value.emptyArray") : value.map((item) => formatActivityDetailValue(item, t)).join(", ");
  try {
    return JSON.stringify(value);
  } catch {
    return t("activity.value.unserializable");
  }
}
    ...
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t("triggerEditor.next", { time: new Date(trigger.nextRunAt).toLocaleString() })
            : trigger.kind === "webhook"
              ? t("triggerKinds.webhook")
              : t("triggerKinds.api")}
        </span>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{t("triggerEditor.fields.label")}</Label>
          <Input
            value={draft.label}
        ...
        {trigger.kind === "schedule" && (
          <div className="md:col-span-2 space-y-1.5">
            <Label className="text-xs">{t("triggerEditor.fields.schedule")}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("triggerEditor.fields.signingMode")}</Label>
              <Select
              ...
              >
                ...
                <SelectContent>
                  {signingModes.map((mode) => (
                    <SelectItem key={mode} value={mode}>{t(`signingModes.${mode}` as const)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("triggerEditor.fields.replayWindowSec")}</Label>
              <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("triggerEditor.lastResult", { value: trigger.lastResult })}</span>}
        <div className="ml-auto flex items-center gap-2">
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("triggerEditor.actions.rotateSecret")}
            </Button>
          )}
          <Button
            variant="outline"
...
export function RoutineDetail() {
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: t("breadcrumbs.routines"), href: "/routines" }, { label: routine.title }]);
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({ title: t("secret.copied", { label }), tone: "success" });
    } catch (error) {
      pushToast({
        title: t("secret.copyFailed", { label: label.toLowerCase() }),
        body: error instanceof Error ? error.message : t("errors.clipboardDenied"),
        tone: "error",
      });
    }
  };
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("toasts.saveFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("toasts.runStarted"), tone: "success" });
      setActiveTab("runs");
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("toasts.runFailed.title"),
        body: error instanceof Error ? error.message : t("toasts.runFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        ...
        body: status === "paused" ? t("toasts.saved.paused") : t("toasts.saved.enabled"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("toasts.updateFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("toasts.addTriggerFailed.title"),
        body: error instanceof Error ? error.message : t("toasts.addTriggerFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("toasts.updateTriggerFailed.title"),
        body: error instanceof Error ? error.message : t("toasts.updateTriggerFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("toasts.deleteTriggerFailed.title"),
        body: error instanceof Error ? error.message : t("toasts.deleteTriggerFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("toasts.webhookSecretRotated"),
        webhookUrl: result.secretMaterial.webhookUrl,
        webhookSecret: result.secretMaterial.webhookSecret,
      });
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        title: t("toasts.rotateWebhookSecretFailed.title"),
        body: error instanceof Error ? error.message : t("toasts.rotateWebhookSecretFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("empty.selectCompany")} />;
  }
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : t("errors.routineNotFound")}
      </p>
    );
  }
  ...
  const automationLabel = routine.status === "archived" ? t("states.archived") : automationEnabled ? t("states.active") : t("states.paused");
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <textarea
          ...
          placeholder={t("fields.routineTitle.placeholder")}
          rows={1}
        ...
        <div className="flex shrink-0 items-center gap-3 pt-1">
          ...
          <button
            ...
            aria-label={automationEnabled ? t("automation.pauseAria") : t("automation.enableAria")}
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("secret.saveNowHint")}</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookUrl} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("secret.labels.webhookUrl"), secretMessage.webhookUrl)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("secret.actions.copyUrl")}
              </Button>
            </div>
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookSecret} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("secret.labels.webhookSecret"), secretMessage.webhookSecret)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("secret.actions.copySecret")}
              </Button>
            </div>
          </div>
        </div>
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          <span>{t("assignment.for")}</span>
          <InlineEntitySelector
            ref={assigneeSelectorRef}
            value={editDraft.assigneeAgentId}
            options={assigneeOptions}
            placeholder={t("selectors.assignee.placeholder")}
            noneLabel={t("selectors.assignee.noneLabel")}
            searchPlaceholder={t("selectors.assignee.searchPlaceholder")}
            emptyMessage={t("selectors.assignee.emptyMessage")}
            ...
            renderTriggerValue={(option) =>
              option ? (
                ...
                <span className="text-muted-foreground">{t("selectors.assignee.label")}</span>
              )
            }
            renderOption={(option) => {
          ...
          />
          <span>{t("assignment.in")}</span>
          <InlineEntitySelector
            ref={projectSelectorRef}
            value={editDraft.projectId}
            options={projectOptions}
            placeholder={t("selectors.project.placeholder")}
            noneLabel={t("selectors.project.noneLabel")}
            searchPlaceholder={t("selectors.project.searchPlaceholder")}
            emptyMessage={t("selectors.project.emptyMessage")}
            ...
            renderTriggerValue={(option) =>
              option && currentProject ? (
                ...
                <span className="text-muted-foreground">{t("selectors.project.label")}</span>
              )
            }
            renderOption={(option) => {
              if (!option.id) return <span className="truncate">{option.label}</span>;
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        ...
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("policies.concurrency.label")}</p>
              <Select
              ...
              >
                ...
                <SelectContent>
                  {concurrencyPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{t(`policies.concurrency.options.${value}` as const)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{t(`policies.concurrency.descriptions.${editDraft.concurrencyPolicy}` as const)}</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("policies.catchUp.label")}</p>
              <Select
              ...
              >
                ...
                <SelectContent>
                  {catchUpPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{t(`policies.catchUp.options.${value}` as const)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{t(`policies.catchUp.descriptions.${editDraft.catchUpPolicy}` as const)}</p>
            </div>
          </div>
        </CollapsibleContent>
      </Collapsible>
      ...
      <div className="flex items-center justify-between">
        {isEditDirty ? (
          <span className="text-xs text-amber-600">{t("saveBar.unsavedChanges")}</span>
        ) : (
          <span />
        )}
        <Button
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-3">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("triggers.addTrigger")}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("triggers.kind")}</Label>
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {t(`triggerKinds.${kind}` as const)}{kind === "webhook" ? ` ${t("triggers.comingSoon")}` : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {newTrigger.kind === "webhook" && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">{t(`signingModeDescriptions.${newTrigger.signingMode}` as const)}</p>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">{t("triggerEditor.fields.replayWindowSec")}</Label>
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? t("actions.adding") : t("actions.addTrigger")}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("triggers.empty")}</p>
          ) : (
