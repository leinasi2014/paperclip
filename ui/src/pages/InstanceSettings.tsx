...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("errors.update"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("errors.unknown");
        throw new Error(
          failures.length === 1
            ? t("errors.disableAllOne", { detail })
            : t("errors.disableAllMany", { count: failures.length, enabled: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("errors.disableAll"));
    },
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span>{t("summary.active", { count: activeCount })}</span>
        <span>{t("summary.disabled", { count: disabledCount })}</span>
        <span>{grouped.length === 1 ? t("summary.company", { count: grouped.length }) : t("summary.companies", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(enabledCount === 1 ? t("confirm.disableAllOne", { count: enabledCount }) : t("confirm.disableAllMany", { count: enabledCount }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("actions.disabling") : t("actions.disableAll")}
          </Button>
        )}
      </div>
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("states.on") : t("states.off")}
                        </Badge>
                        <Link
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("labels.fullAgentConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("actions.disableHeartbeat") : t("actions.enableHeartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
