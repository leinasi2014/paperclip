...
export function CliAuthPage() {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("cliAuth.unavailable.title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {challengeQuery.error instanceof Error ? challengeQuery.error.message : t("cliAuth.unavailable.description")}
          </p>
        </div>
      </div>
    );
  ...
  if (!challenge) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("cliAuth.unavailable.inline")}</div>;
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            {t("cliAuth.labels.command")}: <span className="font-mono text-foreground">{challenge.command}</span>
          </p>
        </div>
      </div>
    );
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-xl font-semibold">
            {challenge.status === "expired" ? t("cliAuth.terminal.expiredTitle") : t("cliAuth.terminal.cancelledTitle")}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("cliAuth.terminal.description")}
          </p>
        </div>
      </div>
    );
  ...
  if (challenge.requiresSignIn || !sessionQuery.data) {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-xl font-semibold">{t("cliAuth.signInRequired.title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("cliAuth.signInRequired.description")}
          </p>
          <Button asChild className="mt-4">
            <Link to={`/auth?next=${encodeURIComponent(currentPath)}`}>{t("cliAuth.actions.signInOrCreate")}</Link>
          </Button>
        </div>
      </div>
    );
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-5 space-y-3 text-sm">
          <div>
            <div className="text-muted-foreground">{t("cliAuth.labels.command")}</div>
            <div className="font-mono text-foreground">{challenge.command}</div>
          </div>
          <div>
            <div className="text-muted-foreground">{t("cliAuth.labels.client")}</div>
            <div className="text-foreground">{challenge.clientName ?? t("cliAuth.labels.defaultClient")}</div>
          </div>
          <div>
            <div className="text-muted-foreground">{t("cliAuth.labels.requestedAccess")}</div>
          ...
          {challenge.requestedCompanyName && (
            <div>
              <div className="text-muted-foreground">{t("cliAuth.labels.requestedCompany")}</div>
        ...
        <div className="mt-5 flex gap-3">
          ...
          >
            {approveMutation.isPending ? t("cliAuth.actions.approving") : t("cliAuth.actions.approve")}
          </Button>
          <Button
            type="button"
            variant="outline"
          ...
          >
            {cancelMutation.isPending ? t("cliAuth.actions.cancelling") : t("cliAuth.actions.cancel")}
          </Button>
        </div>
      </div>
    </div>
