        ...
        {Object.entries(data).map(([key, value]) => (
          <div key={key} className="contents">
            <dt className="text-muted-foreground whitespace-nowrap py-0.5">
              {getFrontmatterFieldLabel(key, t)}
            </dt>
            <dd className="py-0.5">
              {Array.isArray(value) ? (
...
function renderImportFileExtra(
  ...
  const actionBadge = node.action ? (
    <span className={cn(
      ...
      {checked ? getActionLabel(node.action) : getActionLabel("skip")}
    </span>
  ) : null;
...
function ImportPreviewPane({
  ...
  const actionLabel = (value: string) =>
    t(`import.actions.badges.${value}`, { defaultValue: value });
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("import.preview.emptyState")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("import.preview.binaryUnavailable")}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("import.conflicts.items", { count: conflicts.length })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("import.conflicts.actions.skipped") : t("import.conflicts.actions.skip")}
                </button>
                ...
                <span className={cn(
                  ...
                  {t(`import.conflicts.kind.${item.kind}`, { defaultValue: item.kind })}
                </span>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("import.conflicts.actions.confirmed")}
                      </>
                    ) : (
                      t("import.conflicts.actions.confirmRename")
                    )}
                  </button>
                )}
              </div>
    ...
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("import.adapters.agents", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
...
async function readLocalPackageZip(
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(t("import.errors.selectZipPackage"));
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(t("import.errors.emptyArchive"));
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText = t("import.local.help");

  useEffect(() => {
    setBreadcrumbs([
      { label: t("import.breadcrumbs.orgChart"), href: "/org" },
      { label: t("import.breadcrumbs.import") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("import.errors.noSourceConfigured"));
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("import.toasts.previewFailed"),
        body: err instanceof Error ? err.message : t("import.toasts.previewFailedBody"),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("import.toasts.importComplete"),
        body: t("import.toasts.importCompleteBody", {
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("import.toasts.importFailed"),
        body: err instanceof Error ? err.message : t("import.toasts.importFailedBody"),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("import.toasts.packageReadFailed"),
        body: err instanceof Error ? err.message : t("import.toasts.packageReadFailedBody"),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("import.emptyState")} />;
  }
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("import.sourceModes.github") },
              { key: "local", icon: Upload, label: t("import.sourceModes.local") },
            ] as const
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {t("import.local.chooseZip")}
              </Button>
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("import.local.summary", {
          ...
          <Field
            label={t("import.fields.githubUrl")}
            hint={t("import.fields.githubUrlHint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder={t("import.fields.githubUrlPlaceholder")}
              onChange={(e) => {
        ...
        <Field
          label={t("import.fields.target")}
          hint={t("import.fields.targetHint")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
          ...
          >
            <option value="new">{t("import.fields.createNewCompany")}</option>
            <option value="existing">
              {t("import.fields.existingCompany", { name: selectedCompany?.name ?? "" })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("import.fields.newCompanyName")}
            hint={t("import.fields.newCompanyNameHint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("import.fields.newCompanyNamePlaceholder")}
            />
          </Field>
        )}

        <Field
          label={t("import.fields.collisionStrategy")}
          hint={t("import.fields.collisionStrategyHint")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("import.fields.collisionRename")}</option>
            <option value="skip">{t("import.fields.collisionSkip")}</option>
            <option value="replace">{t("import.fields.collisionReplace")}</option>
          </select>
        </Field>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                {t("import.preview.selected", { count: selectedCount, total: totalFiles })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("import.preview.conflicts", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("import.preview.errors", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ...
                : t("import.actions.importFiles", { count: selectedCount })}
            </Button>
          </div>
          ...
          <div className="grid h-[calc(100vh-16rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
            <aside className="flex flex-col border-r border-border overflow-hidden">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("import.preview.packageFiles")}</h2>
              </div>
                ...
                <PackageFileTree
                  ...
                  renderFileExtra={(node, checked) =>
                    renderImportFileExtra(
                      ...
                      checked,
                      renameMap,
                      (action) => t(`import.actions.badges.${action}`, { defaultValue: action }),
                    )}
