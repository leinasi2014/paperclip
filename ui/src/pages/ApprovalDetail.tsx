...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("detail.fallbackTitle") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.approve")),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.reject")),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.revision")),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.resubmit")),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.comment")),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.deleteAgent")),
  });
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("detail.banner.reviewLinkedIssues")
              : t("detail.banner.reviewLinkedIssue"),
          to: `/issues/${primaryLinkedIssue.identifier ?? primaryLinkedIssue.id}`,
        }
      : linkedAgentId
        ? {
            label: t("detail.banner.openHiredAgent"),
            to: `/agents/${linkedAgentId}`,
          }
        : {
            label: t("detail.banner.backToApprovals"),
            to: "/approvals",
          };
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("detail.banner.title")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">{t("detail.banner.description")}</p>
              </div>
            </div>
            <Button
              size="sm"
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("detail.linkedIssues.title")}</p>
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("detail.linkedIssues.description")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("detail.budgetNotice.prefix")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>{t("detail.budgetNotice.suffix")}
            </p>
          )}
          {approval.status === "pending" && (
            ...
            >
              {t("detail.actions.requestRevision")}
            </Button>
          )}
          {approval.status === "revision_requested" && (
            ...
            >
              {t("detail.actions.markResubmitted")}
            </Button>
          )}
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("detail.confirmDeleteDisapprovedAgent"))) return;
                deleteAgentMutation.mutate(linkedAgentId);
              }}
              disabled={deleteAgentMutation.isPending}
            >
              {t("detail.actions.deleteDisapprovedAgent")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("detail.comments.title", { count: comments?.length ?? 0 })}</h3>
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={tCommon("roles.board", { defaultValue: "Board" })} size="sm" />
                )}
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("detail.comments.posting") : t("detail.comments.post")}
          </Button>
        </div>
      </div>
    </div>
