...
export function InstanceExperimentalSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("experimental.errors.update"));
    },
  });
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            type="button"
            data-slot="toggle"
            aria-label={t("experimental.isolatedWorkspaces.toggleLabel")}
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("experimental.autoRestart.title")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("experimental.autoRestart.description")}
            </p>
          </div>
          <button
            type="button"
            data-slot="toggle"
            aria-label={t("experimental.autoRestart.toggleLabel")}
            disabled={toggleMutation.isPending}
            className={cn(
              "relative inline-flex h-5 w-9 items-center rounded-full transition-colors disabled:cursor-not-allowed disabled:opacity-60",
