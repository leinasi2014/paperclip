...
const surfaceOptions: Array<{
  ...
  {
    id: "detail",
    labelKey: "runTranscriptUxLab.surfaces.detail.label",
    eyebrowKey: "runTranscriptUxLab.surfaces.detail.eyebrow",
    descriptionKey: "runTranscriptUxLab.surfaces.detail.description",
    icon: MonitorCog,
  },
  {
    id: "live",
    labelKey: "runTranscriptUxLab.surfaces.live.label",
    eyebrowKey: "runTranscriptUxLab.surfaces.live.eyebrow",
    descriptionKey: "runTranscriptUxLab.surfaces.live.description",
    icon: RadioTower,
  },
  {
    id: "dashboard",
    labelKey: "runTranscriptUxLab.surfaces.dashboard.label",
    eyebrowKey: "runTranscriptUxLab.surfaces.dashboard.eyebrow",
    descriptionKey: "runTranscriptUxLab.surfaces.dashboard.description",
    icon: PanelsTopLeft,
  },
];
    ...
    <div className="overflow-hidden rounded-xl border border-border/70 bg-background/80 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
      <div className="border-b border-border/60 bg-background/90 px-5 py-4">
        ...
        <div className="mt-2 text-sm font-medium">
          {transcriptLabel} ({runTranscriptFixtureEntries.length})
        </div>
      </div>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-2 text-[11px] text-muted-foreground">
                {streaming ? liveNowLabel : finishedLabel}
              </div>
            </div>
...
export function RunTranscriptUxLab() {
  ...
  const resolvedSurfaceOptions = surfaceOptions.map((option) => ({
    ...option,
    label: t(option.labelKey),
    eyebrow: t(option.eyebrowKey),
    description: t(option.descriptionKey),
  }));
  const selected = resolvedSurfaceOptions.find((option) => option.id === selectedSurface) ?? resolvedSurfaceOptions[0];
          ...
          <aside className="border-b border-border/60 bg-background/75 p-5 lg:border-b-0 lg:border-r">
            <div className="mb-5">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/25 bg-cyan-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-cyan-700 dark:text-cyan-300">
                ...
                {t("runTranscriptUxLab.badges.uxLab")}
              </div>
              <h1 className="mt-4 text-2xl font-semibold tracking-tight">{t("runTranscriptUxLab.header.title")}</h1>
              <p className="mt-2 text-sm text-muted-foreground">
                {t("runTranscriptUxLab.header.description")}
              </p>
            </div>

            <div className="space-y-2">
              {resolvedSurfaceOptions.map((option) => {
          ...
          <main className="min-w-0 p-5">
            <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className="rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.18em]">
                  {t("runTranscriptUxLab.badges.sourceRun", { id: runTranscriptFixtureMeta.sourceRunId.slice(0, 8) })}
                </Badge>
                <Badge variant="outline" className="rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.18em]">
            ...
            <div className="mb-5 flex flex-wrap items-center gap-2">
              <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("runTranscriptUxLab.controls.label")}
              </span>
              <div className="inline-flex rounded-full border border-border/70 bg-background/80 p-1">
                {(["nice", "raw"] as const).map((mode) => (
                  ...
                  >
                    {t(`runTranscriptUxLab.controls.modes.${mode}`)}
                  </button>
                ))}
              </div>
              <div className="inline-flex rounded-full border border-border/70 bg-background/80 p-1">
                {(["comfortable", "compact"] as const).map((nextDensity) => (
                  ...
                  >
                    {t(`runTranscriptUxLab.controls.densities.${nextDensity}`)}
                  </button>
                ))}
              </div>
              <Button
              ...
              >
                {streaming
                  ? t("runTranscriptUxLab.controls.actions.showSettled")
                  : t("runTranscriptUxLab.controls.actions.showStreaming")}
              </Button>
            </div>
            ...
            {selectedSurface === "detail" ? (
              <div className={cn(density === "compact" && "max-w-5xl")}>
                <RunDetailPreview
                  ...
                  streaming={streaming}
                  density={density}
                  title={t("runTranscriptUxLab.surfaces.detail.label")}
                  transcriptLabel={t("runTranscriptUxLab.labels.transcript")}
                />
              </div>
              ...
              <div className={cn(density === "compact" && "max-w-4xl")}>
                <LiveWidgetPreview
                  ...
                  mode={detailMode}
                  density={density}
                  liveRunsLabel={t("runTranscriptUxLab.labels.liveRuns")}
                  description={t("runTranscriptUxLab.labels.liveDescription")}
                  openRunLabel={t("runTranscriptUxLab.labels.openRun")}
                />
              </div>
            ) : (
              <DashboardPreview
                ...
                mode={detailMode}
                density={density}
                liveNowLabel={t("runTranscriptUxLab.labels.liveNow")}
                finishedLabel={t("runTranscriptUxLab.labels.finishedAgo")}
              />
            )}
          </main>
        </div>
