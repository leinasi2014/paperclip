...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("settingsPage.detailsTitle") },
    ]);
  }, [selectedCompany?.name, setBreadcrumbs, plugin, t]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("settingsPage.loading")}</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("states.noDescription");
    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant={statusVariant} className="ml-2">
            {t(`states.${displayStatus}`, { defaultValue: displayStatus })}
          </Badge>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: t("settingsPage.tabs.configuration") },
            { value: "status", label: t("settingsPage.tabs.status") },
          ]}
          value={activeTab}
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t("settingsPage.configuration.about.title")}</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("settingsPage.configuration.about.descriptionLabel")}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("settingsPage.configuration.about.authorLabel")}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{t("settingsPage.configuration.about.categoriesLabel")}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("settingsPage.configuration.about.none")}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("settingsPage.configuration.settings.title")}</h2>
              </div>
              {hasCustomSettingsPage ? (
                ...
                <p className="text-sm text-muted-foreground">
                  {t("settingsPage.configuration.settings.noSettings")}
                </p>
              )}
            </section>
          </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("settingsPage.status.runtime.title")}
                  </CardTitle>
                  <CardDescription>
                    {t("settingsPage.status.runtime.description")}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("settingsPage.status.runtime.workerProcess")}
                        </h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("settingsPage.status.labels.status")}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("settingsPage.status.labels.pid")}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("settingsPage.status.labels.uptime")}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("settingsPage.status.labels.pendingRpcs")}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t("settingsPage.status.labels.crashes")}
                                  </span>
                                  <span className="text-xs">
                                    {t("settingsPage.status.labels.crashSummary", {
                                ...
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("settingsPage.status.labels.lastCrash")}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("settingsPage.status.runtime.noWorker")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("settingsPage.status.runtime.recentJobRuns")}
                        </h3>
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("settingsPage.status.runtime.noJobRuns")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("settingsPage.status.runtime.recentWebhookDeliveries")}
                        </h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("settingsPage.status.runtime.noWebhookDeliveries")}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("settingsPage.status.runtime.lastChecked", { time: new Date(dashboardData.checkedAt).toLocaleTimeString() })}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("settingsPage.status.runtime.unavailable")}
                    </p>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-1.5">
                      ...
                      {t("settingsPage.status.logs.title")}
                    </CardTitle>
                    <CardDescription>{t("settingsPage.status.logs.count", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {t("settingsPage.status.health.title")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t("settingsPage.status.health.checking")}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("settingsPage.status.health.overall")}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("settingsPage.status.health.lifecycle")}</span>
                        <Badge variant={statusVariant}>{t(`states.${displayStatus}`, { defaultValue: displayStatus })}</Badge>
                      </div>
                      <p>{t("settingsPage.status.health.lifecycleHint")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t("settingsPage.details.title")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{t("settingsPage.details.pluginId")}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("settingsPage.details.pluginKey")}</span>
                    <span className="font-mono text-xs text-right">{plugin.pluginKey}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("settingsPage.details.npmPackage")}</span>
                    <span className="max-w-[170px] truncate text-right text-xs" title={plugin.packageName}>
                      {plugin.packageName}
                    </span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("settingsPage.details.version")}</span>
              ...
              <Card>
                ...
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("settingsPage.permissions.none")}</p>
                  )}
                </CardContent>
              </Card>
            </div>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("settingsPage.configuration.form.messages.saved") });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("settingsPage.configuration.form.messages.saveFailed") });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("settingsPage.configuration.form.messages.testPassed") });
      } else {
        setTestResult({ type: "error", text: result.message || t("settingsPage.configuration.form.messages.testFailed") });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("settingsPage.configuration.form.messages.testFailed") });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
        ...
        {t("settingsPage.configuration.form.loading")}
      </div>
    );
  }
        ...
        >
          {saveMutation.isPending ? (
            <>
              ...
              {t("settingsPage.configuration.form.actions.saving")}
            </>
          ) : (
            t("settingsPage.configuration.form.actions.save")
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("settingsPage.configuration.form.actions.testing")}
              </>
            ) : (
              t("settingsPage.configuration.form.actions.test")
            )}
          </Button>
        )}
      </div>
...
function formatRelativeTime(isoString: string): string {
  ...
  if (diffMs < 0) return new Intl.RelativeTimeFormat(undefined, { numeric: "auto" }).format(0, "second");
